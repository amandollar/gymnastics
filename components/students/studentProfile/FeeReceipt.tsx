...
export function FeeReceipt({ data }: { data: ReceiptData }) {
  const { invoiceNumber, paidAt, amount, method, notes, student, studentPlan } = data;
  ...
  const pricePerClass = studentPlan.totalSessions > 0 
    ? formatINRReceipt(Math.round(studentPlan.fee / studentPlan.totalSessions))
    : "—";

  // Calculate fee breakdown (Base Fee -> Discount -> Total Fee)
  const finalFee = studentPlan.fee;
  const originalFee = discountPercent > 0 
  ...
  return (
    ...
    >
      {/* ── Top Row: Logo & Title ── */}
      <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "6mm" }}>
        <img
          src="/logo.webp"
          alt="TAG Logo"
          style={{
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "12mm" }}>
        <div style={{ textAlign: "right", fontSize: "12px", color: "#334155", lineHeight: "1.5" }}>
          ...
          <p style={{ margin: 0, fontWeight: 600 }}>Contact no: 7977177463 / 7757965651</p>
        </div>
      </div>
      ...
      <div style={{ marginBottom: "8mm", fontSize: "14px", color: "#0f172a" }}>
        <table style={{ borderCollapse: "collapse" }}>
          <tbody>
            <tr>
              <td style={{ padding: "4px 0", fontWeight: 700, width: "120px" }}>{student.name}</td>
              <td style={{ padding: "4px 0" }}>Age: {age} yr</td>
            </tr>
            <tr>
              <td style={{ padding: "4px 0", color: "#475569" }}>Student ID:</td>
              <td style={{ padding: "4px 0", fontWeight: 600 }}>TAG{String(student.studentNumber).padStart(3, "0")}</td>
            </tr>
            {student.parentName && (
              <tr>
                <td style={{ padding: "4px 0", color: "#475569" }}>Parent Name:</td>
                <td style={{ padding: "4px 0", fontWeight: 600 }}>{student.parentName}</td>
              </tr>
            )}
            <tr>
              <td style={{ padding: "4px 0", color: "#475569" }}>Contact No:</td>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
          <thead>
            <tr style={{ borderBottom: "2px solid #0f172a", borderTop: "2px solid #0f172a" }}>
              <th style={{ padding: "10px 0", textAlign: "left", fontWeight: 700, width: "60px" }}>Sr no.</th>
              <th style={{ padding: "10px 8px", textAlign: "left", fontWeight: 700 }}>Description</th>
              <th style={{ padding: "10px 8px", textAlign: "center", fontWeight: 700, width: "120px" }}>No. of Class</th>
              <th style={{ padding: "10px 0", textAlign: "right", fontWeight: 700, width: "100px" }}>Fees</th>
            </tr>
          </thead>
          <tbody>
            {/* Main Plan Item Row */}
            <tr style={{ verticalAlign: "top" }}>
              ...
              <td style={{ padding: "16px 8px" }}>
                ...
                <div style={{ fontSize: "12px", color: "#475569", lineHeight: "1.6" }}>
                  <p style={{ margin: 0 }}>Duration: {startStr} - {endStr}</p>
              ...
              <td style={{ padding: "16px 0", textAlign: "right", fontWeight: 700 }}>
            ...
            {discountPercent > 0 && (
              <>
                <tr style={{ verticalAlign: "top" }}>
                  <td></td>
                  <td style={{ padding: "8px 8px", fontSize: "12px", color: "#16a34a", fontWeight: 600 }}>
                    {discountPercent}% Discount applied
                  </td>
                  <td></td>
                  <td style={{ padding: "8px 0", textAlign: "right", fontSize: "13px", color: "#16a34a", fontWeight: 700 }}>
                ...
                <tr style={{ borderTop: "1px dashed #cbd5e1" }}>
                  ...
                  <td></td>
                  <td style={{ padding: "12px 0", textAlign: "right", fontWeight: 700, fontSize: "14px" }}>
          ...
          {isPaidCompletely ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
              <p style={{ margin: 0, fontWeight: 700 }}>
                Total Paid: <span style={{ fontSize: "18px", marginLeft: "10px" }}>{formatINRReceipt(finalFee)}</span>
              </p>
              <span style={{ fontWeight: 700, color: "#16a34a", fontSize: "12px", marginTop: "4px" }}>
                Payment Completed (No Dues)
              </span>
            </div>
          ) : (
            <table style={{ borderCollapse: "collapse", fontSize: "13px", color: "#334155" }}>
              <tbody>
                <tr>
                  <td style={{ padding: "2px 0", textAlign: "left", width: "140px" }}>Plan Price:</td>
                  <td style={{ padding: "2px 0", textAlign: "right", fontWeight: 600 }}>{formatINRReceipt(finalFee)}</td>
                </tr>
                <tr>
                  <td style={{ padding: "2px 0", textAlign: "left", fontWeight: 700, color: "#0f172a" }}>Amount Paid:</td>
                  <td style={{ padding: "2px 0", textAlign: "right", fontWeight: 750, color: "#0f172a", fontSize: "14px" }}>{formatINRReceipt(amount)}</td>
                </tr>
                <tr style={{ borderTop: "1px dashed #cbd5e1" }}>
                  <td style={{ padding: "6px 0 2px 0", textAlign: "left", fontWeight: 700, color: "#e11d48" }}>Dues Remaining:</td>
                  <td style={{ padding: "6px 0 2px 0", textAlign: "right", fontWeight: 750, color: "#e11d48" }}>{formatINRReceipt(remainingDues)}</td>
                </tr>
              </tbody>
            </table>
          )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", zIndex: 10 }}>
        ...
        <div style={{ maxWidth: "60%", fontSize: "13px" }}>
          <h3 style={{ margin: "0 0 6px", fontSize: "15px", fontWeight: 700 }}>Notes</h3>
          <p style={{ margin: 0, color: "#334155", fontStyle: "italic", lineHeight: "1.5", whiteSpace: "pre-line" }}>
            {notes || "Fees once paid are non-refundable.\nPlease renew on or before expiry date."}
          </p>
        </div>
        ...
        <div style={{ textAlign: "center", width: "200px" }}>
          ...
          />
          <p style={{ margin: "0 0 2px", fontSize: "11px", fontWeight: 800, color: "#0f172a", letterSpacing: "0.5px" }}>
            ACADEMY OWNER
          </p>
