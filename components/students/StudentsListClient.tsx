    ...
    <div className="space-y-6 min-w-0">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {canManage && (
          <Link
            href="/students/new"
            className="inline-flex w-full sm:w-auto items-center justify-center rounded-lg bg-brand-orange-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-orange-600"
          >
            Add student
          </Link>
        )}
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-orange-500/20"
        />
        <select
          value={statusFilter}
          onChange={(e) =>
      ...
      <div className="md:hidden space-y-2">
        {filtered.length === 0 ? (
          <p className="text-center text-sm text-zinc-500 py-10">No students found.</p>
        ) : (
          filtered.map((s) => (
            <Link
              key={s.id}
            ...
            >
              ...
              {s.activePlan ? (
                ...
                <p className="mt-2 text-xs text-zinc-400">No plan assigned</p>
              )}
            </Link>
          ))
        )}
      ...
      {/* Desktop table */}
      <div className="hidden md:block rounded-lg border border-zinc-200 bg-white shadow-sm overflow-x-auto">
              ...
              filtered.map((s) => (
                <tr key={s.id} className="hover:bg-zinc-50/80">
                  ...
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-zinc-500">{s.parentName}</p>
                  </td>
