...
export async function getSalarySlipDataAction(
  coachId: string,
  year: number,
  month: number
  ...
  try {
    ...
    const { getCoachById, getCoachEarnings } = await import("@/lib/services/coaches");
    const { getAcademyProfile } = await import("@/lib/services/academy");
    const { getMonthSalaryMultiplier } = await import("@/lib/utils/salary");
    const { prisma } = await import("@/lib/prisma");
    ...
    const data = {
      ...
      paidAt: salaryPayment?.paidAt ? salaryPayment.paidAt.toISOString() : null,
      ptStudentsBreakdown,
    };
    ...
    return {
      success: false,
      message: e instanceof Error ? e.message : "Failed to load salary slip data",
    };
  }
}
